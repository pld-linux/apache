...
Summary(pl.UTF-8):	Moduł Apache'a do uruchamiania skryptów CGI
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
Summary(pl.UTF-8):	Moduł Apache'a identyfikujący użytkowników na podstawie plików tekstowych
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
Requires:	htpasswd
...
Summary(pl.UTF-8):	Moduł Apache'a oferujący autoryzację użytkownika "anonimowego"
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
Summary(pl.UTF-8):	Moduł Apache'a oferujący autoryzację użytkowników poprzez LDAP
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
%description mod_auth_ldap -l pl.UTF-8
...
Summary(pl.UTF-8):	Moduł Apache'a z mechanizmem identyfikacji korzystającym z plików DBM
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
Summary(pl.UTF-8):	Moduł Apache'a do autoryzacji MD5
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
Uruchamianie zewnętrznych skryptów CGI za pomocą demona CGI.
...
Moduł umożliwiający obsługę plików .map (imap-file handler).
...
Summary(pl.UTF-8):	Moduł Apache'a umożliwiający logowanie liczby bajtów odebranych oraz wysłanych
Group:		Networking/Daemons
Requires:	%{name} = %{version}-%{release}
...
Moduł pozwalający logować ile bajtów zostało wysłanych lub odebranych
w danym zapytaniu.
